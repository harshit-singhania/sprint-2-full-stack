...
import { ReceiptResponse } from './order.service';
...
export interface DashboardGraphResponse {
  ...
  carsListedPerDay: DailyCount[];
  orderStatusBreakdown: StatusBreakdown[];
  carApprovalBreakdown: StatusBreakdown[];
}
...
export class AdminService {
  ...
  replyToTicket(id: number, message: string): Observable<any> {
    return this.http.post(`${this.base}/api/user/support-tickets/${id}/messages`, { message }).pipe(catchError(e => throwError(() => e)));
  }
  ...
  getAdminReceipt(orderId: number): Observable<ReceiptResponse> {
    return this.http.get<ReceiptResponse>(`${this.base}/api/admin/orders/${orderId}/receipt`)
      .pipe(catchError(e => throwError(() => e)));
  }

  getAllOrders(): Observable<Order[]> {
    return this.http.get<Order[]>(`${this.base}/api/admin/orders`)
      .pipe(catchError(e => throwError(() => e)));
  }

  getDashboardGraph(): Observable<DashboardGraphResponse> {
    return this.http.get<DashboardGraphResponse>(`${this.base}/api/admin/dashboard/graph`)
      .pipe(catchError(e => throwError(() => e)));
  }

  getUserById(userId: number): Observable<User> {
    return this.http.get<User>(`${this.base}/api/admin/users/${userId}`)
      .pipe(catchError(e => throwError(() => e)));
  }

  editUser(userId: number, data: Partial<{ name: string; phoneNumber: string; email: string; newPassword: string }>): Observable<User> {
    return this.http.patch<User>(`${this.base}/api/admin/users/${userId}`, data)
      .pipe(catchError(e => throwError(() => e)));
  }
}
