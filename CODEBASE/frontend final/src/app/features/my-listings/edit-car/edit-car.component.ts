...
export class EditCarComponent implements OnInit {
    ...
    this.carService.getCarById(this.carId).subscribe({
      next: (car) => {
        this.form.patchValue({
          ...
          insured:        car.insured        ?? true,
          pucValid:       car.pucValid       ?? true,
  ...
  onSubmit() {
    ...
    this.carService.updateCar(this.carId, raw).subscribe({
      next: () => {
        this.toast.success('Listing updated — pending admin re-approval.');
        this.router.navigate(['/my-listings']);
      },
      error: (err) => {
        this.isSaving = false;
